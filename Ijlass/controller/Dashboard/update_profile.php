...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email)) {
        ...
        window.location.href = '/view/Dashboard/?page=Profile';</script>";
        exit;
    }
    ...
    if (!empty($_POST['oldPassword']) && !empty($_POST['newPassword'])) {
        ...
        $passwordCheckQuery = "SELECT password FROM tbl_users WHERE email = ?";
    ...
    if (!empty($fieldsToUpdate)) {
        $query = "UPDATE tbl_users SET " . implode(", ", $fieldsToUpdate) . " WHERE email = ?";
        ...
        if ($stmt->execute()) {
            echo "<script>window.location.href = '/view/Dashboard/?page=profilesuccess';</script>";
        } else {
