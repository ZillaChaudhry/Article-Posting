...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare("SELECT password, name, profile_pic, email, 'user' role FROM tbl_admin WHERE email = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            echo "<script>
                    window.location.href = '/view/Admin_Dashboard/?page=admindashboard';
                  </script>";
        } else {
            // Password mismatch
            echo "<script>
